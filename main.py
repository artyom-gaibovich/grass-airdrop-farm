...
async def connect_to_wss(socks5_proxy, user_id, file_name):
    ...
    while True:
        try:
            ...
            if any(error_msg in str(e) for error_msg in ["Host unreachable", "[SSL: WRONG_VERSION_NUMBER]", "invalid length of packed IP address string", "Empty connect reply", "Device creation limit exceeded", "sent 1011 (internal error) keepalive ping timeout; no close frame received"]):
                ...
                remove_proxy_from_list(socks5_proxy, file_name)
                return None
            else:
                continue
...
async def main(file_name, user_id):
    with open(file_name, 'r') as file:
        all_proxies = file.read().splitlines()

    active_proxies = random.sample(all_proxies, min(1000, len(all_proxies)))
    tasks = {asyncio.create_task(connect_to_wss(proxy, user_id, file_name)): proxy for proxy in active_proxies}
...
def remove_proxy_from_list(proxy, file_name):
    with open(file_name, "r+") as file:
