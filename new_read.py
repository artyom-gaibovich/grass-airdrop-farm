...
def clear_output_folder(output_folder):
    # Очищаем папку output, удаляя все файлы
...
def process_files(input_folder, output_folder):
    # Проверяем, существуют ли папки, если нет - создаём
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # Очищаем папку output от старых файлов
    clear_output_folder(output_folder)

    # Получаем список всех текстовых файлов в папке input
    input_files = [f for f in os.listdir(input_folder) if f.endswith(".txt")]

    # Сортируем файлы по имени, чтобы обрабатывать их по порядку
    input_files.sort()
